...
TEST(Point, test_constructor) {
  ...
  EXPECT_DOUBLE_EQ(actual[0].x(), 1);
  EXPECT_DOUBLE_EQ(actual[0].y(), 2);
  EXPECT_DOUBLE_EQ(actual[1].x(), 2);
  EXPECT_DOUBLE_EQ(actual[1].y(), 6);
  EXPECT_DOUBLE_EQ(actual[2].x(), -3);
  EXPECT_DOUBLE_EQ(actual[2].y(), 0.3);
}
...
TEST(Point, test_mult) {
  ...
  cv::Mat H = (cv::Mat_<double>(3, 3) << 0.89125166, 0.62396362, 0.24830436,
