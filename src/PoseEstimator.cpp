...
namespace wip {
std::pair<double, cv::Mat> PoseEstimator::estimate(
  ...
  for (size_t i = 0; i < ransacN_; i++) {
    ...
    std::vector<cv::Point2d> srcMatchedPoints, dstMatchedPoints;
    ...
    std::vector<cv::Point2d> evalSrcMatchedPoints, evalDstMatchedPoints;
...
cv::Mat compositeProjectionMatrix(const cv::Mat &K, const cv::Mat &R,
                                  const cv::Mat &t) {
  cv::Mat outerMat = cv::Mat::zeros(3, 4, CV_64F);
...
bool checkRules(const cv::Mat &p3d, const cv::Mat &R, const cv::Mat &t) {
  ...
  if (parallax < 0.99 && reconstructedPoint1.at<double>(2, 0) < 0) {
    return false;
  }

  cv::Mat reconstructedPoint2 = R_ * reconstructedPoint1 + t_;
  reconstructedPoint2.convertTo(reconstructedPoint2, CV_64F);

  if (parallax < 0.99 && reconstructedPoint2.at<double>(2, 0) < 0) {
    return false;
  }
  return true;
}
  ...
  for (size_t i = 0; i < rotations.size(); i++) {
    ...
    for (size_t j = 0; j < inliners_.size(); j++) {
      ...
      const cv::Mat projectedPoint1 =
        P1 * x3DHomo / cv::Mat(P1 * x3DHomo).at<double>(2, 0);
      const cv::Mat projectedPoint2 =
        P2 * x3DHomo / cv::Mat(P2 * x3DHomo).at<double>(2, 0);

      const auto x1 = projectedPoint1.at<double>(0, 0);
      const auto y1 = projectedPoint1.at<double>(1, 0);
      const auto x2 = projectedPoint2.at<double>(0, 0);
      const auto y2 = projectedPoint2.at<double>(1, 0);
